class RepaymentUI {
   ...
   set repayment_balance(value) {
      ...
      this.#repayment_element_refference.input_repayment_balance.value = final_value;
   }
   set repayment_date_interval(value) {
      const sanitized_value = this.#html_dom.sanitize(value);
      this.#repayment_element_refference.select_repayment_date_interval.value = sanitized_value
   }
   ...
   set trickle_amount_payable(value) {
      ...
      const parsed_value = parseFloat(sanitized_value);
      const final_value = isNaN(parsed_value) ? 0 : parsed_value;
      this.#repayment_element_refference.input_trickle_amount_payable.value = final_value;
   }
   set trickle_overdue_balance(value) {
      const sanitized_value = this.#html_dom.sanitize(value);
      const parsed_value = parseFloat(sanitized_value);
      const final_value = isNaN(parsed_value) ? 0 : parsed_value;
      this.#repayment_element_refference.input_trickle_overdue_balance.value = final_value;
   }
   set trickle_repayment_amount(value) {
      const sanitized_value = this.#html_dom.sanitize(value);
      const parsed_value = parseFloat(sanitized_value);
      const final_value = isNaN(parsed_value) ? 0 : parsed_value;
      this.#repayment_element_refference.input_trickle_repayment_amount.value = final_value;
   }
   set trickle_repayment_starting_date(value) {
      const sanitized_value = this.#html_dom.sanitize(value);
      const parsed_value = parseFloat(sanitized_value);
      const final_value = isNaN(parsed_value) ? 0 : parsed_value;
      this.#repayment_element_refference.input_repayment_starting_date.value = final_value;
   }
   set trickle_repayment_date(value) {
      const sanitized_value = this.#html_dom.sanitize(value);
      const parse_date = AU_EST_Time(sanitized_value);
      this.#repayment_element_refference.input_trickle_repayment_date.valueAsDate = parse_date
   }
   set trickle_repayment_interval(value) {
      const sanitized_value = this.#html_dom.sanitize(value);
      this.#repayment_element_refference.select_trickle_repayment_interval.value = sanitized_value;
   }

   get repayment_balance() {
      const sanitized_value = this.#html_dom.sanitize(this.#repayment_element_refference.input_repayment_balance.value)
      const parsed_value = parseFloat(sanitized_value);
      const final_value = isNaN(parsed_value) ? 0 : parsed_value;
      return final_value
   }
